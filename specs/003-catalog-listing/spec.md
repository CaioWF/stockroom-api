...
- As an integrator, I want the catalog closed to callers without a credential, so that reading it
  is a deliberate grant rather than an open endpoint. Accepted when a request with no valid token
...
- FR1: A `GET /products` route returns the products in the catalog. The catalog is one shared
  collection; the caller's token does not select which products are returned. See
  [ADR-0007](../../docs/architecture/adr/0007-one-shared-catalog-partition.md).
...
- FR3: Products are stored under one fixed partition dedicated to the catalog, disjoint from the
  profile and session-token partitions.
...
- FR8: A client that supplies `cursor` receives the products that follow that position in the
  order of FR4.
- FR9: The cursor is opaque to the client and carries no partition information. With one fixed
  partition there is none to carry.
- FR10: A cursor is accepted only when it positively matches its required shape — the product sort
  ...
  fail.
- FR10a: A well-formed cursor naming a position that no longer exists — a product since removed —
  is honored rather than rejected: the listing returns whatever follows that position. It is not an
  error and it costs no extra query.
...
- AC-6: Withdrawn by [ADR-0007](../../docs/architecture/adr/0007-one-shared-catalog-partition.md).
  Cross-account isolation was a consequence of a product decision that no longer holds. The number
  is retired rather than reused, so older references still resolve.
...
- AC-7a: Withdrawn by [ADR-0007](../../docs/architecture/adr/0007-one-shared-catalog-partition.md),
  for the same reason as AC-6. A cursor is no longer foreign to anything.
...
- Any authorization model finer than "a valid token may read". There is one catalog and every
  authenticated caller sees all of it — see
  [ADR-0007](../../docs/architecture/adr/0007-one-shared-catalog-partition.md).
...
- Distinguishing a cursor whose product was removed from one that never existed. Per FR10a both
  are honored rather than rejected, which costs no extra query and cannot read anything the caller
