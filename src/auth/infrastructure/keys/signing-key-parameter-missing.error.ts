 ...
 * `ParameterStoreSigningKeyProvider.fetchParameterValue` throws this when
 * the configured SSM parameter resolves with no `Value` at all — a
 ...
 */
export class SigningKeyParameterMissingError extends Error {
  constructor(public readonly parameterName: string) {
    super(`SSM parameter ${parameterName} has no value`);
    this.name = 'SigningKeyParameterMissingError';
  }
}
